...
Page({
  ...
  bindPickerChange_pet(e) {
    console.log('picker发送选择改变，携带值为', e.detail.value)
    this.setData({
      image_url:'https://duogesi.cn/pet/pics/dog2.png',
  ...
  sumbit:function(e){
    ...
    if (this.data.contact_name !== "" && this.data.contact_phone !== "" && this.data.contact_addree !== "" && this.data.dogName !== "" && this.data.dogName !== "第一次下单请先建立宠物档案"&& this.data.price !== "" && this.data.date !== "" && this.data.time !== "" && app.globalData.openid != "") {
      wx.request({
        url: 'https://duogesi.cn/pet/congcong/bathingaction_execute.action',
        data: {
        ...
        fail: function (res) {
          wx.showToast({
            ...
            image: 'https://duogesi.cn/pet/pics/fail.png',
  ...
  prepay:function(that){
    ...
    var first=[];
    wx.request({
      url: 'https://duogesi.cn/pet/congcong/dogaction_preload.action',
      method: 'GET',
      header: {
        //'content-type': 'application/json' // 默认值
        "Content-Type": "application/x-www-form-urlencoded"
